...
public class anim16 extends JPanel implements Runnable, KeyListener, MouseMotionListener {
	/**
	 * 
	 */
	...
	Point2D mousedif = new Point2D(0,0);

	public static void main(String[] args) {
		anim16 a = new anim16(1080, 720, 6, 4);
		new Thread(a).start();
		new Thread(a).start();

	}

	public anim16(int width, int height, int arwidth, int arheight) {
	...
	public void update() {
		...
		prevmouse = mouse;
		mousedif = Point2D.add(prevmouse,mouse.scale(-1)).scale(100);
		System.out.println(prevmouse.x+"-"+mouse.x+"="+(prevmouse.x-mouse.x));
		...
		for (Asteroid a : particles) {
			...
			for(Asteroid b:particles){
				if(a!=b&&a!=particles.get(0)){
					Point2D difference = Point2D.add(a.pos, b.pos.scale(-1));
					double dist = difference.dist();
					double effect = Math.min(10/(dist*dist),2);
					a.vel = Point2D.add(a.vel.scale(1-effect),b.vel.scale(effect));
				}
			}
		}
